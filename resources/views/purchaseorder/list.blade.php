<div class="heading-buttons main-left">
	<div class="buttons pull-left">
		...
		<a href="{{URL}}/purchaseorders/entry" class="btn btn-small btn-primary btn-icon "><i class="fa fa-list-alt"></i> Chi tiết</a>
		<a href="javascript:window.open('{{URL}}/purchaseorders/export-pdf-list','_blank');" class="btn btn-small btn-primary btn-icon "><i class="fa fa-print"></i> Xuất PDF</a>
	...
	<div class="buttons pull-right">
		...
		<div class="pagination pagination-small pull-right" style="margin: -15px 20px 5px 20px;">
			<?php  echo $list_purchaseorder->render(); ?>
		</div>
	</div>
</div>

<form action="{{URL}}/purchaseorders/list" method="POST" accept-charset="utf-8" id="form-list">
...
<table class="table table-bordered table-condensed table-striped table-primary table-vertical-center table-list-view">
	...
	<tbody>
		@foreach($list_purchaseorder as $key => $po)
		<tr class="selectable" data-id="{{$po['id']}}">
			<td>{{($po['id'])}}</td>
...
<script>
	$(function(){
		...
		$('.table-list-view tbody tr').on("click",function(){
			...
			window.location = '{{URL}}/purchaseorders/entry/'+id;
		})
	})
</script>