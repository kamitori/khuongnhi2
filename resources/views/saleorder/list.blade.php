<div class="heading-buttons main-left">
	...
	<div class="buttons pull-right">
		...
		<div class="pagination pagination-small pull-right" style="margin: -15px 20px 5px 20px;">
			<?php  echo $list_saleorder->render(); ?>
		</div>
	</div>
</div>

<form action="{{URL}}/saleorders/list" method="POST" accept-charset="utf-8" id="form-list">
...
<table class="table table-bordered table-condensed table-striped table-primary table-vertical-center table-list-view">
	<thead>
		<tr class="small">
			...
			<th style="width: 20%" data-column="sum_invest" {{ isset($arr_sort['sum_invest'])?'data-sort='.$arr_sort['sum_invest']:'' }}>Tổng tiền</th>
	...
	<tbody>
		...
		<tr class="selectable" data-id="{{$rpo['id']}}">
			...
			<td data-type="currency">{{$rpo['sum_amount']!=null?$rpo['sum_amount']:0}}</td>
...
<script>
	$(function(){
		...
		$('.table-list-view tbody tr').on("click",function(){
			...
			window.location = '{{URL}}/saleorders/entry/'+id;
		})
	})
</script>